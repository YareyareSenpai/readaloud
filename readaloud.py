#!/usr/bin/env python3
"""
readaloud — EPUB/TXT reader with multi-backend neural TTS
Backends: edge-tts (online), kokoro, piper, f5-tts (offline)
Deps: ebooklib (pip), edge-tts (pipx), ffplay (ffmpeg)
Usage: readaloud [file]   or just `readaloud` for file picker
       readaloud --debug  to start with debug overlay on

Reading Scenarios (auto-selected by panel visibility):
  Scenario 1 — neither panel visible   → 1 bordered reading column
  Scenario 2 — exactly one panel       → 2 snake-flow columns
  Scenario 3 — both panels visible     → 3 snake-flow columns
"""

import os, sys, re, json, shutil, subprocess, tempfile, threading, time, signal
import curses, traceback, logging
from pathlib import Path
from html.parser import HTMLParser
from typing import Dict, List, Any, Optional

# ─── Logging (debug file, never pollutes curses) ──────────────────────────────

LOG_PATH = Path.home() / ".config" / "readaloud" / "debug.log"
LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
logging.basicConfig(
    filename=str(LOG_PATH),
    level=logging.DEBUG,
    format="%(asctime)s %(levelname)-7s %(message)s",
    datefmt="%H:%M:%S",
)
log = logging.getLogger("readaloud")

# ─── Config paths ─────────────────────────────────────────────────────────────

CONFIG_DIR  = Path.home() / ".config" / "readaloud"
VOICES_DIR  = CONFIG_DIR / "voices"
MODELS_DIR  = CONFIG_DIR / "models"
CONFIG_PATH = CONFIG_DIR / "config.json"

for _d in (CONFIG_DIR, VOICES_DIR, MODELS_DIR):
    _d.mkdir(parents=True, exist_ok=True)

# ─── Backend system ───────────────────────────────────────────────────────────

class BaseTTSBackend:
    """Shared interface all TTS engines implement."""
    def generate_chunk(self, text: str, voice: str, speed: float, output_path: str) -> bool:
        raise NotImplementedError


class EdgeTTSBackend(BaseTTSBackend):
    """Microsoft Edge neural TTS via pipx run edge-tts (online)."""
    def generate_chunk(self, text: str, voice: str, speed: float, output_path: str) -> bool:
        try:
            subprocess.run(
                ["pipx", "run", "edge-tts",
                 "--voice", voice, "--text", text, "--write-media", output_path],
                check=True, capture_output=True, timeout=60)
            return True
        except Exception as e:
            log.error("EdgeTTS error: %s", e)
            return False


class KokoroBackend(BaseTTSBackend):
    """Kokoro offline TTS — fast, high-quality CPU inference.
    Tries in order:
      1. native 'kokoro' (hexgrad, KPipeline) — requires Python <3.13
      2. 'pykokoro' (pure-Python ONNX, no version cap) — build_pipeline API
      3. CLI fallback
    Pipeline is cached per (voice, speed) so ONNX loads once per session.
    last_error is set on failure so callers can surface the real message.
    """
    def __init__(self):
        self.last_error = ""
        self._pykokoro_pipeline = None   # cached pykokoro pipeline
        self._pykokoro_key      = None   # (voice, speed) key for cache

    def _get_pykokoro_pipeline(self, voice: str, speed: float):
        """Return a cached pykokoro pipeline, rebuilding only when voice/speed changes."""
        key = (voice, speed)
        if self._pykokoro_pipeline is not None and self._pykokoro_key == key:
            return self._pykokoro_pipeline
        from pykokoro import build_pipeline, PipelineConfig, GenerationConfig
        cfg = PipelineConfig(
            voice=voice,
            generation=GenerationConfig(speed=speed),
        )
        pipeline = build_pipeline(config=cfg, eager=True)
        self._pykokoro_pipeline = pipeline
        self._pykokoro_key      = key
        log.info("pykokoro: pipeline built for voice=%s speed=%s", voice, speed)
        return pipeline

    def generate_chunk(self, text: str, voice: str, speed: float, output_path: str) -> bool:
        import traceback as _tb
        self.last_error = ""

        # 1. Native kokoro (Python <3.13)
        try:
            from kokoro import KPipeline
            import soundfile as sf
            lang_code = "a" if voice.startswith("am_") or voice.startswith("af_") else "b"
            pipeline  = KPipeline(lang_code=lang_code)
            generator = pipeline(text, voice=voice, speed=speed, split_pattern=r"\n+")
            for _, _, audio in generator:
                sf.write(output_path, audio, 24000)
                return True
            self.last_error = "kokoro: generator produced no audio"
            return False
        except ImportError:
            log.debug("kokoro not available, trying pykokoro")
        except Exception as e:
            self.last_error = f"kokoro: {type(e).__name__}: {e}"
            log.error("kokoro (native) error:\n%s", _tb.format_exc())
            return False

        # 2. pykokoro — cached pipeline (ONNX loads once per voice/speed)
        try:
            pipeline = self._get_pykokoro_pipeline(voice, speed)
            result   = pipeline(text)
            result.save_wav(output_path)
            return True
        except ImportError:
            log.debug("pykokoro not available, trying CLI")
        except Exception as e:
            self.last_error = f"pykokoro: {type(e).__name__}: {e}"
            log.error("pykokoro error:\n%s", _tb.format_exc())
            # Invalidate cache on error so next call retries fresh
            self._pykokoro_pipeline = None
            self._pykokoro_key      = None
            return False

        # 3. CLI last resort
        try:
            subprocess.run(
                ["kokoro", "--voice", voice, "--text", text, "--output", output_path],
                check=True, capture_output=True, timeout=120)
            return True
        except Exception as e:
            self.last_error = f"kokoro CLI: {type(e).__name__}: {e}"
            log.error("Kokoro CLI error: %s", e)
            return False


class PiperBackend(BaseTTSBackend):
    """Piper offline TTS — ultra-lightweight ONNX runtime."""
    def generate_chunk(self, text: str, voice: str, speed: float, output_path: str) -> bool:
        model_path = MODELS_DIR / f"{voice}.onnx"
        if not model_path.exists():
            log.error("Piper model not found: %s", model_path)
            return False
        try:
            proc = subprocess.Popen(
                ["piper", "--model", str(model_path), "--output_file", output_path],
                stdin=subprocess.PIPE,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
            proc.communicate(input=text.encode("utf-8"))
            return proc.returncode == 0
        except Exception as e:
            log.error("Piper error: %s", e)
            return False


class F5TTSBackend(BaseTTSBackend):
    """F5-TTS flow-matching voice cloner (slow on CPU, GPU recommended).
    Requires: ~/.config/readaloud/voices/ref.wav (+ optional ref.txt)
    """
    def generate_chunk(self, text: str, voice: str, speed: float, output_path: str) -> bool:
        ref_audio = VOICES_DIR / "ref.wav"
        ref_text_file = VOICES_DIR / "ref.txt"
        if not ref_audio.exists():
            log.error("F5-TTS: ref.wav not found at %s", ref_audio)
            return False
        ref_text = ""
        if ref_text_file.exists():
            ref_text = ref_text_file.read_text().strip()
        out_dir = Path(output_path).parent
        try:
            subprocess.run(
                ["f5-tts_infer-cli",
                 "--ref_audio",  str(ref_audio),
                 "--ref_text",   ref_text or "Reference voice sample.",
                 "--gen_text",   text,
                 "--output_dir", str(out_dir)],
                check=True, capture_output=True, timeout=600)
            # CLI writes out.wav — rename to expected path
            default_out = out_dir / "out.wav"
            if default_out.exists():
                default_out.rename(output_path)
                return True
            log.error("F5-TTS: out.wav not produced")
            return False
        except Exception as e:
            log.error("F5-TTS error: %s", e)
            return False


# ─── System scanner ───────────────────────────────────────────────────────────

ENGINE_META: Dict[str, Dict[str, str]] = {
    "edge-tts": {
        "label": "Edge TTS",
        "type":  "online",
        "note":  "Microsoft neural voices (needs internet)",
        "ext":   ".mp3",
    },
    "kokoro": {
        "label": "Kokoro",
        "type":  "offline",
        "note":  "Fast, high-quality CPU engine",
        "ext":   ".wav",
    },
    "piper": {
        "label": "Piper",
        "type":  "offline",
        "note":  "Ultra-light ONNX, needs model in ~/.config/readaloud/models/",
        "ext":   ".wav",
    },
    "f5-tts": {
        "label": "F5-TTS",
        "type":  "offline",
        "note":  "Voice cloner — slow on CPU, needs ref.wav",
        "ext":   ".wav",
    },
}


class SystemScanner:
    """Runs once at startup; discovers which TTS backends are usable."""

    @staticmethod
    def scan() -> Dict[str, bool]:
        status: Dict[str, bool] = {k: False for k in ENGINE_META}

        if not (shutil.which("ffmpeg") and shutil.which("ffplay")):
            log.warning("ffmpeg/ffplay missing — no audio possible")
            return status

        # edge-tts: pipx available or direct import
        try:
            import edge_tts  # noqa: F401
            status["edge-tts"] = True
        except ImportError:
            if shutil.which("edge-tts") or shutil.which("pipx"):
                status["edge-tts"] = True

        # kokoro — accepts native kokoro, pykokoro (Python 3.13+ ONNX wrapper), or CLI
        try:
            import kokoro  # noqa: F401
            status["kokoro"] = True
        except ImportError:
            try:
                import pykokoro  # noqa: F401
                status["kokoro"] = True
            except ImportError:
                if shutil.which("kokoro"):
                    status["kokoro"] = True

        # piper
        if shutil.which("piper"):
            status["piper"] = True
        else:
            try:
                import piper  # noqa: F401
                status["piper"] = True
            except ImportError:
                pass

        # f5-tts
        if shutil.which("f5-tts_infer-cli") or shutil.which("f5-tts"):
            status["f5-tts"] = True
        else:
            try:
                import f5_tts  # noqa: F401
                status["f5-tts"] = True
            except ImportError:
                pass

        log.info("Scanner: %s", {k: v for k, v in status.items()})
        return status


class MasterPlaybackManager:
    """Routes synthesis calls to the active backend."""

    def __init__(self):
        self.availability = SystemScanner.scan()
        self._backends: Dict[str, BaseTTSBackend] = {
            "edge-tts": EdgeTTSBackend(),
            "kokoro":   KokoroBackend(),
            "piper":    PiperBackend(),
            "f5-tts":   F5TTSBackend(),
        }

    def available_engines(self) -> List[str]:
        return [k for k in ENGINE_META if self.availability.get(k)]

    def is_available(self, engine: str) -> bool:
        return self.availability.get(engine, False)

    def output_ext(self, engine: str) -> str:
        return ENGINE_META.get(engine, {}).get("ext", ".mp3")

    def synthesize(self, engine: str, text: str, voice: str,
                   speed: float, dest_path: str) -> bool:
        if not self.is_available(engine):
            log.warning("Engine %s unavailable, falling back to edge-tts", engine)
            engine = "edge-tts"
        return self._backends[engine].generate_chunk(text, voice, speed, dest_path)


# Singleton — created once at startup
_manager: Optional[MasterPlaybackManager] = None


def get_manager() -> MasterPlaybackManager:
    global _manager
    if _manager is None:
        _manager = MasterPlaybackManager()
    return _manager


# ─── Dependency check ─────────────────────────────────────────────────────────

def check_deps():
    missing = []
    try:
        import ebooklib  # noqa: F401
    except ImportError:
        missing.append("ebooklib  →  pip install --break-system-packages ebooklib")
    if not shutil.which("ffplay"):
        missing.append("ffplay    →  sudo pacman -S ffmpeg")
    # edge-tts is the fallback — warn if neither pipx nor the module is present
    try:
        r = subprocess.run(["pipx", "run", "edge-tts", "--version"],
                           capture_output=True, timeout=30)
        if r.returncode != 0:
            missing.append("edge-tts  →  pipx install edge-tts")
    except (FileNotFoundError, subprocess.TimeoutExpired):
        try:
            import edge_tts  # noqa: F401
        except ImportError:
            missing.append("edge-tts  →  pipx install edge-tts  (pacman -S python-pipx)")
    if missing:
        print("\033[1;31m✗ Missing dependencies:\033[0m")
        for m in missing:
            print(f"  {m}")
        sys.exit(1)


# ─── EPUB / TXT parsing ───────────────────────────────────────────────────────

class _HTMLStripper(HTMLParser):
    def __init__(self):
        super().__init__()
        self._parts, self._skip = [], False
    def handle_starttag(self, tag, attrs):
        if tag in ("script", "style"):
            self._skip = True
        if tag in ("p","h1","h2","h3","h4","li","br","div"):
            self._parts.append("\n")
    def handle_endtag(self, tag):
        if tag in ("script","style"):
            self._skip = False
    def handle_data(self, data):
        if not self._skip:
            self._parts.append(data)
    def get_text(self):
        return re.sub(r'\n{3,}', '\n\n', "".join(self._parts)).strip()

def _html_to_text(html_bytes):
    p = _HTMLStripper()
    try:
        p.feed(html_bytes.decode("utf-8", errors="replace"))
    except Exception:
        pass
    return p.get_text()

def parse_epub(path):
    from ebooklib import epub, ITEM_DOCUMENT
    book  = epub.read_epub(path, options={"ignore_ncx": False})
    spine = [item_id for item_id, _ in book.spine]
    out   = []
    for iid in spine:
        item = book.get_item_with_id(iid)
        if item is None or item.get_type() != ITEM_DOCUMENT:
            continue
        text = _html_to_text(item.get_content())
        if len(text.strip()) < 80:
            continue
        title = item.get_name().replace("/", " › ")
        m = re.search(r'^\s*(.{3,80})', text)
        if m:
            fl = m.group(1).strip()
            if len(fl) < 80:
                title = fl
        out.append((title, text))
    return out

def parse_txt(path):
    text   = Path(path).read_text(errors="replace")
    chunks = re.split(r'\n{3,}|(?m)^(Chapter\s+\w+.*)', text)
    out, buf, n = [], [], 1
    for chunk in chunks:
        if chunk is None:
            continue
        buf.append(chunk)
        if len("\n".join(buf)) > 8000:
            out.append((f"Part {n}", "\n".join(buf).strip()))
            buf, n = [], n + 1
    if buf:
        out.append((f"Part {n}", "\n".join(buf).strip()))
    return [(t, c) for t, c in out if len(c.strip()) > 80]


# ─── Constants & config ───────────────────────────────────────────────────────

# Edge-TTS voices (used only when engine == "edge-tts")
EDGE_VOICES = [
    ("Aria",     "US", "F", "en-US-AriaNeural"),
    ("Jenny",    "US", "F", "en-US-JennyNeural"),
    ("Michelle", "US", "F", "en-US-MichelleNeural"),
    ("Guy",      "US", "M", "en-US-GuyNeural"),
    ("Davis",    "US", "M", "en-US-DavisNeural"),
    ("Sonia",    "GB", "F", "en-GB-SoniaNeural"),
    ("Maisie",   "GB", "F", "en-GB-MaisieNeural"),
    ("Ryan",     "GB", "M", "en-GB-RyanNeural"),
    ("Natasha",  "AU", "F", "en-AU-NatashaNeural"),
    ("William",  "AU", "M", "en-AU-WilliamNeural"),
    ("Clara",    "CA", "F", "en-CA-ClaraNeural"),
    ("Liam",     "CA", "M", "en-CA-LiamNeural"),
]

# Kokoro voices (short names; full IDs passed as voice string)
KOKORO_VOICES = [
    ("af_heart",  "US", "F", "af_heart"),
    ("af_bella",  "US", "F", "af_bella"),
    ("af_nicole", "US", "F", "af_nicole"),
    ("am_adam",   "US", "M", "am_adam"),
    ("am_michael","US", "M", "am_michael"),
    ("bf_emma",   "GB", "F", "bf_emma"),
    ("bm_george", "GB", "M", "bm_george"),
]

# Piper and F5 don't have static voice lists; they use model file names / ref.wav
ENGINE_VOICES: Dict[str, list] = {
    "edge-tts": EDGE_VOICES,
    "kokoro":   KOKORO_VOICES,
    "piper":    [],   # populated at runtime from MODELS_DIR
    "f5-tts":   [],   # single cloned voice from ref.wav
}

SPEEDS      = [0.75, 0.9, 1.0, 1.1, 1.25, 1.5, 1.75, 2.0]
ZOOM_LEVELS = [0, 2, 5, 9, 14, 20, 28]
ZOOM_DEFAULT = 0
ALIGN_ORDER  = ["left", "center", "right"]
THEME_ORDER  = ["terminal", "dark", "gruvbox", "nord", "solarized"]
ENGINE_ORDER = list(ENGINE_META.keys())

# F5-TTS animation frames (shown while generating)
F5_ANIM = ["◐ Generating", "◓ Generating", "◑ Generating", "◒ Generating"]

THEMES = {
    "terminal": {
        "name": "Terminal",
        "pairs": {
            "normal":  (curses.COLOR_WHITE,   -1),
            "accent":  (curses.COLOR_CYAN,    -1),
            "sel":     (curses.COLOR_BLACK,    curses.COLOR_CYAN),
            "playing": (curses.COLOR_YELLOW,  -1),
            "ok":      (curses.COLOR_GREEN,   -1),
            "err":     (curses.COLOR_RED,     -1),
            "dim":     (curses.COLOR_MAGENTA, -1),
            "hdr":     (curses.COLOR_WHITE,    curses.COLOR_BLUE),
            "ftr":     (curses.COLOR_BLACK,    curses.COLOR_WHITE),
            "hl":      (curses.COLOR_BLACK,    curses.COLOR_YELLOW),
            "bm":      (curses.COLOR_BLACK,    curses.COLOR_GREEN),
            "btn":     (curses.COLOR_BLACK,    curses.COLOR_CYAN),
            "rpanel":  (curses.COLOR_WHITE,    -1),
            "offline": (curses.COLOR_GREEN,   -1),
            "online":  (curses.COLOR_CYAN,    -1),
        },
    },
    "dark": {
        "name": "Dark Navy",
        "256": {
            "bg":(80,90,120),"fg":(900,910,940),"accent":(400,780,980),
            "yellow":(980,800,200),"green":(300,860,500),"red":(980,350,350),
            "dim":(650,550,800),"hdr_bg":(50,60,90),"hl_bg":(800,700,100),
            "rp_bg":(60,70,100),
        },
        "pairs": {
            "normal":(16,17),"accent":(18,17),"sel":(17,18),
            "playing":(19,17),"ok":(20,17),"err":(21,17),"dim":(22,17),
            "hdr":(16,23),"ftr":(17,16),"hl":(17,24),"bm":(17,20),
            "btn":(17,18),"rpanel":(16,25),"offline":(20,17),"online":(18,17),
        },
    },
    "gruvbox": {
        "name": "Gruvbox",
        "256": {
            "bg":(156,141,100),"fg":(980,920,760),"accent":(980,740,180),
            "yellow":(980,740,180),"green":(690,730,200),"red":(980,360,220),
            "dim":(800,550,380),"hdr_bg":(120,110,80),"hl_bg":(980,740,180),
            "rp_bg":(130,120,90),
        },
        "pairs": {
            "normal":(16,17),"accent":(19,17),"sel":(17,19),
            "playing":(19,17),"ok":(20,17),"err":(21,17),"dim":(22,17),
            "hdr":(16,23),"ftr":(17,16),"hl":(17,24),"bm":(17,20),
            "btn":(17,19),"rpanel":(16,25),"offline":(20,17),"online":(19,17),
        },
    },
    "nord": {
        "name": "Nord",
        "256": {
            "bg":(184,204,239),"fg":(840,870,910),"accent":(530,750,870),
            "yellow":(920,796,545),"green":(638,745,549),"red":(749,380,416),
            "dim":(700,600,780),"hdr_bg":(149,168,203),"hl_bg":(920,796,545),
            "rp_bg":(160,180,215),
        },
        "pairs": {
            "normal":(16,17),"accent":(18,17),"sel":(17,18),
            "playing":(19,17),"ok":(20,17),"err":(21,17),"dim":(22,17),
            "hdr":(16,23),"ftr":(17,16),"hl":(17,24),"bm":(17,20),
            "btn":(17,18),"rpanel":(16,25),"offline":(20,17),"online":(18,17),
        },
    },
    "solarized": {
        "name": "Solarized Dark",
        "256": {
            "bg":(0,168,211),"fg":(513,580,588),"accent":(149,545,823),
            "yellow":(709,537,0),"green":(521,600,0),"red":(862,196,184),
            "dim":(423,443,768),"hdr_bg":(0,121,168),"hl_bg":(709,537,0),
            "rp_bg":(0,100,140),
        },
        "pairs": {
            "normal":(16,17),"accent":(18,17),"sel":(17,18),
            "playing":(19,17),"ok":(20,17),"err":(21,17),"dim":(22,17),
            "hdr":(16,23),"ftr":(17,16),"hl":(17,24),"bm":(17,20),
            "btn":(17,18),"rpanel":(16,25),"offline":(20,17),"online":(18,17),
        },
    },
}

PAIR_IDX = {
    "normal":1,"accent":2,"sel":3,"playing":4,"ok":5,
    "err":6,"dim":7,"hdr":8,"ftr":9,"hl":10,"bm":11,"btn":12,"rpanel":13,
    "offline":14,"online":15,
}

BOX = {"tl":"╭","tr":"╮","bl":"╰","br":"╯","h":"─","v":"│","ml":"├","mr":"┤"}

STATE_ICONS = {
    "idle":"○","loading":"◌","playing":"▶","paused":"⏸","done":"✓","error":"✗",
    "generating":"◐",
}

_KITTY_INC   = b"\x1b]30001\x1b\\"
_KITTY_DEC   = b"\x1b]30002\x1b\\"
_KITTY_RESET = b"\x1b]30003\x1b\\"


def load_config():
    if CONFIG_PATH.exists():
        try:
            return json.loads(CONFIG_PATH.read_text())
        except Exception:
            pass
    return {
        "voice_index": 0,
        "speed_index": 2,
        "theme": "terminal",
        "align": "left",
        "zoom": ZOOM_DEFAULT,
        "last_file": None,
        "bookmarks": {},
        "engine": "edge-tts",
    }

def save_config(cfg):
    CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
    CONFIG_PATH.write_text(json.dumps(cfg, indent=2))

def _kitty_font(seq: bytes):
    try:
        with open("/dev/tty", "wb", buffering=0) as tty:
            tty.write(seq)
    except OSError:
        pass


# ─── Voice helpers ────────────────────────────────────────────────────────────

def _piper_voices() -> list:
    """Scan MODELS_DIR for .onnx files and return as voice tuples."""
    voices = []
    try:
        for f in sorted(MODELS_DIR.glob("*.onnx")):
            name = f.stem
            voices.append((name, "??", "?", name))
    except Exception:
        pass
    return voices

def get_voices_for_engine(engine: str) -> list:
    if engine == "piper":
        v = _piper_voices()
        return v if v else [("(no models)", "", "", "")]
    if engine == "f5-tts":
        ref = VOICES_DIR / "ref.wav"
        label = "ref.wav ✓" if ref.exists() else "ref.wav missing"
        return [(label, "", "", "ref")]
    return ENGINE_VOICES.get(engine, [])

def current_voice_id(cfg: dict) -> str:
    """Return the voice identifier string for the active engine."""
    engine = cfg.get("engine", "edge-tts")
    voices = get_voices_for_engine(engine)
    if not voices:
        return ""
    idx = cfg.get("voice_index", 0)
    idx = min(idx, len(voices) - 1)
    return voices[idx][3]  # 4th element is the ID


# ─── Text helpers ─────────────────────────────────────────────────────────────

def split_text(text, max_chars=4500):
    sentences = re.split(r'(?<=[.!?])\s+', text)
    chunks, buf, length = [], [], 0
    for s in sentences:
        if length + len(s) > max_chars and buf:
            chunks.append(" ".join(buf))
            buf, length = [], 0
        buf.append(s)
        length += len(s) + 1
    if buf:
        chunks.append(" ".join(buf))
    return [c for c in chunks if c.strip()]

def wrap_text(text, width, align):
    lines = []
    for para in text.split("\n"):
        para = para.strip()
        if not para:
            lines.append(""); continue
        while len(para) > width:
            cut = para.rfind(" ", 0, width)
            if cut == -1: cut = width
            lines.append(para[:cut])
            para = para[cut:].lstrip()
        lines.append(para)
    if align == "right":
        return [l.rjust(width) if l else "" for l in lines]
    if align == "center":
        return [l.center(width) if l else "" for l in lines]
    return lines


# ─── Player ───────────────────────────────────────────────────────────────────

class Player:
    def __init__(self):
        self.state         = "idle"
        self.chunk_idx     = 0
        self.total_chunks  = 0
        self.error_msg     = ""
        self.est_char_pos  = 0
        self.play_start_ts = 0.0
        self.chapter_len   = 0
        self._chars_per_sec = 11.67
        self._proc         = None
        self._thread       = None
        self._stop_evt     = threading.Event()
        self._pause_evt    = threading.Event()
        self._pause_ts     = 0.0
        self._paused_total = 0.0
        self.on_done       = None
        # F5-TTS animation
        self._anim_idx     = 0
        self._anim_ts      = 0.0

    @property
    def anim_frame(self) -> str:
        return F5_ANIM[self._anim_idx % len(F5_ANIM)]

    def _tick_anim(self):
        now = time.time()
        if now - self._anim_ts > 0.25:
            self._anim_idx += 1
            self._anim_ts = now

    def play(self, text: str, voice: str, speed: float, engine: str):
        self.stop()
        self._stop_evt.clear()
        self._pause_evt.clear()
        self._paused_total = 0.0
        self.chapter_len   = len(text)
        self.est_char_pos  = 0
        self._thread = threading.Thread(
            target=self._run, args=(text, voice, speed, engine), daemon=True)
        self._thread.start()

    def _run(self, text: str, voice: str, speed: float, engine: str):
        mgr = get_manager()
        is_f5 = (engine == "f5-tts")
        self.state = "generating" if is_f5 else "loading"
        self.error_msg = ""
        tmpdir = tempfile.mkdtemp(prefix="readaloud_")
        log.debug("Player._run engine=%s voice=%s speed=%s len=%d", engine, voice, speed, len(text))

        chunks = split_text(text)
        self.total_chunks = len(chunks)
        audio_files = []
        ext = mgr.output_ext(engine)

        for i, chunk in enumerate(chunks):
            if self._stop_evt.is_set():
                break
            self.chunk_idx = i
            if is_f5:
                self._tick_anim()
            outfile = os.path.join(tmpdir, f"chunk_{i:04d}{ext}")
            ok = mgr.synthesize(engine, chunk, voice, speed, outfile)
            if not ok:
                self.state = "error"
                backend = mgr._backends.get(engine)
                detail = getattr(backend, "last_error", "") or f"synthesis failed on chunk {i+1}"
                self.error_msg = f"{engine}: {detail}"
                log.error(self.error_msg)
                self._cleanup(tmpdir); return
            if not os.path.exists(outfile):
                self.state = "error"
                self.error_msg = f"{engine}: no output file produced"
                log.error(self.error_msg)
                self._cleanup(tmpdir); return
            audio_files.append(outfile)
            log.debug("chunk %d/%d generated", i+1, len(chunks))

        if self._stop_evt.is_set() or not audio_files:
            self._cleanup(tmpdir)
            self.state = "idle"; return

        playlist = os.path.join(tmpdir, "playlist.txt")
        with open(playlist, "w") as f:
            for af in audio_files:
                f.write(f"file '{af}'\n")

        # Speed filter: edge-tts uses atempo; wav backends rely on atempo too
        tempo = speed
        chain = []
        while tempo > 2.0:
            chain.append("atempo=2.0"); tempo /= 2.0
        chain.append(f"atempo={tempo:.3f}")

        self.state = "playing"
        self.play_start_ts = time.time()
        self._chars_per_sec = 140 / 60 * 5 * speed
        log.debug("playback start chars_per_sec=%.1f", self._chars_per_sec)

        try:
            self._proc = subprocess.Popen(
                ["ffplay", "-nodisp", "-autoexit", "-loglevel", "quiet",
                 "-f", "concat", "-safe", "0", "-i", playlist,
                 "-af", ",".join(chain)],
                stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
            while self._proc.poll() is None:
                if self._stop_evt.is_set():
                    self._proc.terminate(); break
                if self._pause_evt.is_set():
                    self._proc.send_signal(signal.SIGSTOP)
                    self.state = "paused"
                    self._pause_ts = time.time()
                    while self._pause_evt.is_set() and not self._stop_evt.is_set():
                        time.sleep(0.1)
                    if not self._stop_evt.is_set():
                        self._paused_total += time.time() - self._pause_ts
                        self._proc.send_signal(signal.SIGCONT)
                        self.state = "playing"
                else:
                    elapsed = time.time() - self.play_start_ts - self._paused_total
                    self.est_char_pos = min(int(elapsed * self._chars_per_sec),
                                           self.chapter_len)
                time.sleep(0.2)
        except Exception as e:
            self.state = "error"; self.error_msg = str(e)
            log.exception("ffplay error")
            self._cleanup(tmpdir); return

        self._cleanup(tmpdir)
        if not self._stop_evt.is_set():
            self.state = "done"
            log.debug("chapter done")
            if self.on_done:
                self.on_done()

    def toggle_pause(self):
        if self.state == "playing":
            self._pause_evt.set()
        elif self.state == "paused":
            self._pause_evt.clear()

    def stop(self):
        self._stop_evt.set()
        self._pause_evt.clear()
        if self._proc and self._proc.poll() is None:
            try: self._proc.terminate()
            except: pass
        if self._thread and self._thread.is_alive():
            self._thread.join(timeout=3)
        self.state = "idle"
        self.est_char_pos = 0
        log.debug("player stopped")

    def play_cached(self, playlist: str, audio_dir: str,
                    chapter_len: int, speed: float):
        """Start playback from a pre-synthesized playlist (cache hit)."""
        self.stop()
        self._stop_evt.clear()
        self._pause_evt.clear()
        self._paused_total = 0.0
        self.chapter_len   = chapter_len
        self.est_char_pos  = 0
        self._thread = threading.Thread(
            target=self._run_cached, args=(playlist, audio_dir, speed), daemon=True)
        self._thread.start()

    def _run_cached(self, playlist: str, audio_dir: str, speed: float):
        """ffplay loop for a pre-built playlist; mirrors end of _run()."""
        tempo = speed
        chain = []
        while tempo > 2.0:
            chain.append("atempo=2.0"); tempo /= 2.0
        chain.append(f"atempo={tempo:.3f}")

        self.state = "playing"
        self.play_start_ts = time.time()
        self._chars_per_sec = 140 / 60 * 5 * speed
        self.error_msg = ""
        log.debug("play_cached: %s", playlist)

        try:
            self._proc = subprocess.Popen(
                ["ffplay", "-nodisp", "-autoexit", "-loglevel", "quiet",
                 "-f", "concat", "-safe", "0", "-i", playlist,
                 "-af", ",".join(chain)],
                stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL)
            while self._proc.poll() is None:
                if self._stop_evt.is_set():
                    self._proc.terminate(); break
                if self._pause_evt.is_set():
                    self._proc.send_signal(signal.SIGSTOP)
                    self.state = "paused"
                    self._pause_ts = time.time()
                    while self._pause_evt.is_set() and not self._stop_evt.is_set():
                        time.sleep(0.1)
                    if not self._stop_evt.is_set():
                        self._paused_total += time.time() - self._pause_ts
                        self._proc.send_signal(signal.SIGCONT)
                        self.state = "playing"
                else:
                    elapsed = time.time() - self.play_start_ts - self._paused_total
                    self.est_char_pos = min(int(elapsed * self._chars_per_sec),
                                           self.chapter_len)
                time.sleep(0.2)
        except Exception as e:
            self.state = "error"; self.error_msg = str(e)
            log.exception("ffplay cached error")
            return

        if not self._stop_evt.is_set():
            self.state = "done"
            log.debug("cached chapter done")
            if self.on_done:
                self.on_done()

    def _cleanup(self, tmpdir):
        try: shutil.rmtree(tmpdir, ignore_errors=True)
        except: pass


# ─── Preload cache ────────────────────────────────────────────────────────────

CACHE_DIR    = CONFIG_DIR / "cache"
PRELOAD_AHEAD = 2   # how many chapters ahead to pre-synthesize

class PreloadCache:
    """
    Pre-synthesizes the next PRELOAD_AHEAD chapters in background threads.
    Cache entries live on disk at CACHE_DIR/<key>/ and survive between
    engine/voice/speed changes only when the key still matches.

    Key format:  {ch_idx}_{engine}_{voice}_{speed:.3f}
    Each entry contains:
        chunk_NNNN.{ext}  — audio files
        playlist.txt      — ffplay concat playlist
        .done             — sentinel written last (entry only usable when present)
    """

    def __init__(self):
        CACHE_DIR.mkdir(parents=True, exist_ok=True)
        self._lock     = threading.Lock()
        self._workers: Dict[int, threading.Thread] = {}   # ch_idx → thread
        self._cancel:  Dict[int, threading.Event]  = {}   # ch_idx → stop event
        # Clean up stale cache entries from previous sessions on startup
        self._gc()

    # ── Public API ────────────────────────────────────────────────────────────

    def key(self, ch_idx: int, engine: str, voice: str, speed: float) -> str:
        safe_voice = re.sub(r'[^\w.-]', '_', voice)
        safe_eng   = re.sub(r'[^\w.-]', '_', engine)
        return f"{ch_idx}_{safe_eng}_{safe_voice}_{speed:.3f}"

    def warm(self, ch_idx: int, engine: str, voice: str, speed: float) -> bool:
        """Return True if a complete cached entry exists for this chapter."""
        entry = CACHE_DIR / self.key(ch_idx, engine, voice, speed)
        return (entry / ".done").exists()

    def get_playlist(self, ch_idx: int, engine: str, voice: str, speed: float) -> Optional[str]:
        """Return path to playlist.txt if cache is warm, else None."""
        entry = CACHE_DIR / self.key(ch_idx, engine, voice, speed)
        pl    = entry / "playlist.txt"
        if (entry / ".done").exists() and pl.exists():
            return str(pl)
        return None

    def prime(self, chapters: list, from_idx: int,
              engine: str, voice: str, speed: float):
        """
        Kick off background synthesis for the next PRELOAD_AHEAD chapters
        starting at from_idx.  Already-warm or already-running entries are
        skipped.  Entries for chapters we no longer need are cancelled.
        """
        wanted = set()
        for offset in range(1, PRELOAD_AHEAD + 1):
            ci = from_idx + offset
            if ci < len(chapters):
                wanted.add(ci)

        with self._lock:
            # Cancel workers we no longer need
            for ci in list(self._workers.keys()):
                if ci not in wanted:
                    evt = self._cancel.get(ci)
                    if evt:
                        evt.set()
                    self._workers.pop(ci, None)
                    self._cancel.pop(ci, None)

            # Start workers for wanted chapters not yet warm or running
            for ci in wanted:
                if self.warm(ci, engine, voice, speed):
                    continue
                if ci in self._workers and self._workers[ci].is_alive():
                    continue
                _, text = chapters[ci]
                evt = threading.Event()
                self._cancel[ci] = evt
                t = threading.Thread(
                    target=self._synthesize,
                    args=(ci, text, engine, voice, speed, evt),
                    daemon=True)
                self._workers[ci] = t
                t.start()
                log.debug("preload: queued ch=%d engine=%s voice=%s", ci, engine, voice)

    def invalidate(self):
        """Cancel all running workers and wipe the cache directory."""
        with self._lock:
            for evt in self._cancel.values():
                evt.set()
            self._workers.clear()
            self._cancel.clear()
        try:
            shutil.rmtree(str(CACHE_DIR), ignore_errors=True)
            CACHE_DIR.mkdir(parents=True, exist_ok=True)
            log.debug("preload: cache invalidated")
        except Exception as e:
            log.warning("preload: invalidate error: %s", e)

    def invalidate_chapter(self, ch_idx: int):
        """Cancel and remove a single chapter's cache entry."""
        with self._lock:
            evt = self._cancel.pop(ch_idx, None)
            if evt:
                evt.set()
            self._workers.pop(ch_idx, None)
        # Remove all keys that start with this chapter index
        prefix = f"{ch_idx}_"
        try:
            for entry in CACHE_DIR.iterdir():
                if entry.name.startswith(prefix):
                    shutil.rmtree(str(entry), ignore_errors=True)
        except Exception:
            pass

    def status(self, ch_idx: int, engine: str, voice: str, speed: float) -> str:
        """Return 'warm'|'loading'|'idle' for debug display."""
        if self.warm(ch_idx, engine, voice, speed):
            return "warm"
        with self._lock:
            t = self._workers.get(ch_idx)
            if t and t.is_alive():
                return "loading"
        return "idle"

    # ── Background worker ─────────────────────────────────────────────────────

    def _synthesize(self, ch_idx: int, text: str, engine: str,
                    voice: str, speed: float, cancel: threading.Event):
        mgr   = get_manager()
        k     = self.key(ch_idx, engine, voice, speed)
        entry = CACHE_DIR / k
        done  = entry / ".done"

        # Another process may have already written this entry
        if done.exists():
            log.debug("preload: ch=%d already warm, skipping", ch_idx)
            return

        # Use a tmp subdir then rename to avoid partial reads
        tmp_entry = CACHE_DIR / (k + ".tmp")
        try:
            shutil.rmtree(str(tmp_entry), ignore_errors=True)
            tmp_entry.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            log.warning("preload: mkdir failed ch=%d: %s", ch_idx, e)
            return

        chunks = split_text(text)
        ext    = mgr.output_ext(engine)
        audio_files = []

        for i, chunk in enumerate(chunks):
            if cancel.is_set():
                shutil.rmtree(str(tmp_entry), ignore_errors=True)
                log.debug("preload: ch=%d cancelled at chunk %d", ch_idx, i)
                return
            outfile = str(tmp_entry / f"chunk_{i:04d}{ext}")
            ok = mgr.synthesize(engine, chunk, voice, speed, outfile)
            if not ok or not os.path.exists(outfile):
                shutil.rmtree(str(tmp_entry), ignore_errors=True)
                log.warning("preload: ch=%d synthesis failed chunk %d", ch_idx, i)
                return
            audio_files.append(outfile)
            log.debug("preload: ch=%d chunk %d/%d done", ch_idx, i+1, len(chunks))

        if cancel.is_set():
            shutil.rmtree(str(tmp_entry), ignore_errors=True)
            return

        # Write playlist using final paths (after rename)
        final_audio = [str(entry / Path(af).name) for af in audio_files]
        pl_path = tmp_entry / "playlist.txt"
        with open(pl_path, "w") as f:
            for af in final_audio:
                f.write(f"file '{af}'\n")

        # Atomic rename tmp → final
        try:
            if entry.exists():
                shutil.rmtree(str(entry), ignore_errors=True)
            tmp_entry.rename(entry)
            (entry / ".done").touch()
            log.info("preload: ch=%d warm (%d chunks, engine=%s)", ch_idx, len(chunks), engine)
        except Exception as e:
            log.warning("preload: rename failed ch=%d: %s", ch_idx, e)
            shutil.rmtree(str(tmp_entry), ignore_errors=True)

    def _gc(self):
        """Remove cache entries older than 2 hours to avoid stale disk use."""
        cutoff = time.time() - 7200
        try:
            for entry in CACHE_DIR.iterdir():
                if entry.is_dir():
                    try:
                        if entry.stat().st_mtime < cutoff:
                            shutil.rmtree(str(entry), ignore_errors=True)
                    except Exception:
                        pass
        except Exception:
            pass


# ─── TUI ──────────────────────────────────────────────────────────────────────

RP_WIDTH = 28   # right panel width when visible (slightly wider for engine labels)

class TUI:
    def __init__(self, stdscr, chapters, filename, cfg, start_debug=False):
        self.scr          = stdscr
        self.chapters     = chapters
        self.filename     = Path(filename).name
        self.cfg          = cfg
        self.ch_idx       = 0
        self.ch_scroll    = 0
        self.view_scroll  = 0
        self.focus        = "chapters"
        self.nav_visible  = True
        self.rpanel       = None         # None | "voices" | "bookmarks"
        self.rp_sel       = 0
        self.rp_scroll    = 0
        # Voices panel has two sub-sections: engine row (0..3) and voice row
        self._rp_section  = "engine"     # "engine" | "voice"
        self._engine_sel  = ENGINE_ORDER.index(cfg.get("engine", "edge-tts"))
        self._voice_sel   = cfg.get("voice_index", 0)
        self.player       = Player()
        self.player.on_done = self._on_chapter_done
        self.running      = True
        self._lock        = threading.Lock()
        self._tlines      = []
        self._t_ch        = -1
        self._t_align     = None
        self._t_width     = -1
        self._autoscroll  = True
        self._highlight   = True
        self._debug       = start_debug
        self._last_key    = ""
        self._last_mouse  = ""
        self._bm_key      = self.filename
        self.cfg.setdefault("bookmarks", {}).setdefault(self._bm_key, [])
        self.cfg.setdefault("engine", "edge-tts")
        self._mgr         = get_manager()
        self._preload     = PreloadCache()
        # Multi-page snake scroll: total line offset across all columns
        self._snake_scroll = 0
        self._setup_colors()
        log.info("TUI init: %s  %d chapters  engine=%s",
                 self.filename, len(chapters), cfg.get("engine"))

    # ── Colors ────────────────────────────────────────────────────────────────

    def _setup_colors(self):
        curses.start_color()
        curses.use_default_colors()
        tn    = self.cfg.get("theme", "terminal")
        theme = THEMES.get(tn, THEMES["terminal"])
        use256 = (curses.COLORS >= 256
                  and curses.can_change_color()
                  and "256" in theme)
        if use256:
            cd = theme["256"]
            defs = [(16,"fg"),(17,"bg"),(18,"accent"),(19,"yellow"),
                    (20,"green"),(21,"red"),(22,"dim"),
                    (23,"hdr_bg"),(24,"hl_bg"),(25,"rp_bg")]
            for idx, key in defs:
                if key in cd:
                    try: curses.init_color(idx, *cd[key])
                    except curses.error: pass
            pairs = theme["pairs"]
        else:
            pairs = THEMES["terminal"]["pairs"]
        for name, pidx in PAIR_IDX.items():
            fg, bg = pairs.get(name, (curses.COLOR_WHITE, -1))
            try: curses.init_pair(pidx, fg, bg)
            except curses.error: pass
        log.debug("colors set theme=%s use256=%s", tn, use256)

    def _cp(self, name, bold=False, dim=False):
        a = curses.color_pair(PAIR_IDX.get(name, 1))
        if bold: a |= curses.A_BOLD
        if dim:  a |= curses.A_DIM
        return a

    # ── Callbacks ─────────────────────────────────────────────────────────────

    def _on_chapter_done(self):
        with self._lock:
            if self.ch_idx < len(self.chapters) - 1:
                self.ch_idx += 1
                self._invalidate()
                self._play()

    def _play(self):
        engine = self.cfg.get("engine", "edge-tts")
        voice  = current_voice_id(self.cfg)
        speed  = SPEEDS[self.cfg["speed_index"]]
        _, text = self.chapters[self.ch_idx]
        log.info("play ch=%d engine=%s voice=%s speed=%s", self.ch_idx, engine, voice, speed)

        pl = self._preload.get_playlist(self.ch_idx, engine, voice, speed)
        if pl:
            log.info("preload: cache HIT ch=%d", self.ch_idx)
            self.player.play_cached(pl, str(CACHE_DIR / self._preload.key(
                self.ch_idx, engine, voice, speed)), len(text), speed)
        else:
            log.info("preload: cache MISS ch=%d — synthesizing live", self.ch_idx)
            self.player.play(text, voice, speed, engine)

        # Prime the next PRELOAD_AHEAD chapters in background
        self._preload.prime(self.chapters, self.ch_idx, engine, voice, speed)

    # ── Text helpers ──────────────────────────────────────────────────────────

    def _invalidate(self):
        self._t_ch = -1
        self._snake_scroll = 0

    def _get_lines(self, width):
        align = self.cfg.get("align", "left")
        if (self._t_ch != self.ch_idx
                or self._t_align != align
                or self._t_width != width):
            _, text = self.chapters[self.ch_idx]
            self._tlines  = wrap_text(text, max(4, width - 2), align)
            self._t_ch    = self.ch_idx
            self._t_align = align
            self._t_width = width
            self.view_scroll = 0
            self._snake_scroll = 0
        return self._tlines

    def _char_to_line(self, pos, lines):
        total = 0
        for i, l in enumerate(lines):
            total += len(l.strip()) + 1
            if total >= pos:
                return i
        return len(lines) - 1

    # ── Reading scenario detection ────────────────────────────────────────────

    def _reading_scenario(self) -> int:
        """
        1 → neither panel visible  (1 bordered column)
        2 → exactly one panel      (2 snake columns)
        3 → both panels visible    (3 snake columns)
        """
        panels = (1 if self.nav_visible else 0) + (1 if self.rpanel else 0)
        if panels == 0:
            return 1
        elif panels == 1:
            return 2
        else:
            return 3

    # ── Layout geometry ───────────────────────────────────────────────────────

    def _layout(self, H, W):
        header_h  = 3
        footer_h  = 6
        content_h = H - header_h - footer_h
        nav_w     = max(20, W // 4) if self.nav_visible else 0
        rp_w      = RP_WIDTH if self.rpanel else 0
        div_l     = 1 if self.nav_visible else 0
        div_r     = 1 if self.rpanel else 0
        text_left = nav_w + div_l
        text_w    = W - text_left - rp_w - div_r
        rp_left   = W - rp_w
        return nav_w, text_left, text_w, rp_left, rp_w, header_h, footer_h, content_h

    # ── Main loop ─────────────────────────────────────────────────────────────

    def run(self):
        curses.curs_set(0)
        self.scr.timeout(200)
        curses.mousemask(curses.ALL_MOUSE_EVENTS | curses.REPORT_MOUSE_POSITION)
        while self.running:
            try:
                self._draw()
            except Exception:
                log.exception("draw error")
            key = self.scr.getch()
            try:
                if key == curses.KEY_MOUSE:
                    self._handle_mouse()
                else:
                    self._handle_key(key)
            except Exception:
                log.exception("key error key=%d", key)

    # ── Draw dispatcher ───────────────────────────────────────────────────────

    def _draw(self):
        self.scr.erase()
        H, W = self.scr.getmaxyx()
        if H < 14 or W < 50:
            self._sa(0, 0, "Terminal too small (need 50×14)", self._cp("err", bold=True))
            self.scr.refresh(); return

        nav_w, tl, tw, rpl, rpw, hh, fh, ch = self._layout(H, W)
        scenario = self._reading_scenario()

        self._draw_header(W)

        if self.nav_visible:
            self._draw_nav(nav_w, hh, ch)
            for r in range(ch):
                self._sa(hh+r, nav_w, BOX["v"], self._cp("accent"))

        # Route to correct reading scenario renderer
        if scenario == 1:
            self._draw_text_scenario1(tl, tw, hh, ch)
        elif scenario == 2:
            self._draw_text_scenario2(tl, tw, hh, ch, W)
        else:
            self._draw_text_scenario3(tl, tw, hh, ch, W)

        if self.rpanel:
            for r in range(ch):
                self._sa(hh+r, rpl-1, BOX["v"], self._cp("accent"))
            if self.rpanel == "voices":
                self._draw_rp_voices(rpl, rpw, hh, ch)
            elif self.rpanel == "bookmarks":
                self._draw_rp_bookmarks(rpl, rpw, hh, ch)

        self._draw_footer(W, H, fh)
        self._draw_float_btns(W, H, fh)
        if self._debug:
            self._draw_debug(H, W)
        self.scr.refresh()

    def _sa(self, y, x, text, attr=0):
        H, W = self.scr.getmaxyx()
        if y < 0 or y >= H or x < 0 or x >= W: return
        ml = W - x - 1
        if ml <= 0: return
        try:
            self.scr.addstr(y, x, text[:ml], attr)
        except curses.error:
            pass

    # ── Header ────────────────────────────────────────────────────────────────

    def _draw_header(self, W):
        bar = " " * (W-1)
        try: self.scr.addstr(0, 0, bar, self._cp("hdr", bold=True))
        except curses.error: pass

        tn      = THEMES.get(self.cfg.get("theme","terminal"),{}).get("name","")
        engine  = self.cfg.get("engine","edge-tts")
        elabel  = ENGINE_META.get(engine,{}).get("label", engine)
        title   = f"  📖 readaloud  ─  {self.filename}"
        nav_ind = "◀▶" if self.nav_visible else "▶▶"
        rp_ind  = ("V" if self.rpanel=="voices" else
                   "B" if self.rpanel=="bookmarks" else "·")
        right   = f"[{rp_ind}] {nav_ind}  {tn}  {self.ch_idx+1}/{len(self.chapters)}  "
        self._sa(0, 0,                              title, self._cp("hdr", bold=True))
        self._sa(0, max(len(title)+2, W-len(right)-1), right, self._cp("hdr", bold=True))
        self._sa(1, 0, BOX["h"]*(W-1), self._cp("accent"))

        vname  = current_voice_id(self.cfg) or "—"
        speed  = SPEEDS[self.cfg["speed_index"]]
        state  = self.player.state
        # F5 animation
        if state == "generating":
            self.player._tick_anim()
            icon = self.player.anim_frame
        else:
            icon = STATE_ICONS.get(state, "○")
        sc = {"playing":    self._cp("playing", bold=True),
              "loading":    self._cp("dim",     bold=True),
              "generating": self._cp("dim",     bold=True),
              "paused":     self._cp("accent",  bold=True),
              "error":      self._cp("err",     bold=True),
              "done":       self._cp("ok"),
              "idle":       self._cp("normal")}.get(state, self._cp("normal"))
        zi   = self.cfg.get("zoom", ZOOM_DEFAULT)
        albl = self.cfg.get("align","left").capitalize()
        auto = "AUTO↓" if self._autoscroll else "─────"
        hl_l = "HL" if self._highlight else "hl"
        dbg  = " DBG" if self._debug else ""
        scn  = f" S{self._reading_scenario()}"
        if state == "generating":
            state_lbl = self.player.anim_frame
        else:
            state_lbl = f"{icon} {state.upper():<10}"
        sub = (f"  {state_lbl}"
               f"  [{elabel}] {vname:<14}"
               f"  Speed:{speed:.2f}×"
               f"  Align:{albl:<6}"
               f"  Z{zi+1}/{len(ZOOM_LEVELS)}"
               f"  {auto}  {hl_l}{dbg}{scn}")
        self._sa(2, 0, sub, sc)

    # ── Left nav (chapters) ───────────────────────────────────────────────────

    def _draw_nav(self, width, top, height):
        focused = self.focus == "chapters"
        lattr   = self._cp("accent", bold=True) if focused else self._cp("dim")
        bms     = self.cfg["bookmarks"].get(self._bm_key, [])
        bm_ch   = {bm["chapter"] for bm in bms}
        self._sa(top, 0, f" CHAPTERS{f'  ♥{len(bms)}' if bms else ''}", lattr)

        visible = height - 1
        if self.ch_idx < self.ch_scroll:
            self.ch_scroll = self.ch_idx
        elif self.ch_idx >= self.ch_scroll + visible:
            self.ch_scroll = self.ch_idx - visible + 1

        for row in range(visible):
            idx = self.ch_scroll + row
            y   = top + 1 + row
            if idx >= len(self.chapters): break
            title, _ = self.chapters[idx]
            is_bm    = idx in bm_ch
            maxw     = width - 5
            disp     = title[:maxw] + "…" if len(title) > maxw else title
            bmark    = "♥" if is_bm else " "

            engine = self.cfg.get("engine", "edge-tts")
            voice  = current_voice_id(self.cfg)
            speed  = SPEEDS[self.cfg["speed_index"]]
            is_warm = (idx != self.ch_idx
                       and self._preload.warm(idx, engine, voice, speed))

            if idx == self.ch_idx and self.player.state in ("playing","loading","paused","generating"):
                icon = STATE_ICONS.get(self.player.state, "▶")
                if self.player.state == "generating":
                    icon = "◐"
                line = f"{bmark}{icon} {disp}"
                attr = self._cp("playing", bold=True)
            elif idx == self.ch_idx and focused:
                line = f"{bmark}› {disp}".ljust(width-1)
                attr = self._cp("sel", bold=True)
            elif idx == self.ch_idx:
                line = f"{bmark}› {disp}"
                attr = self._cp("accent", bold=True)
            elif is_warm:
                line = f"{bmark}⚡ {disp[:width-5]}"
                attr = self._cp("ok")
            else:
                line = f"{bmark}{idx+1:>3}. {disp[:width-7]}"
                attr = self._cp("bm") if is_bm else self._cp("normal")

            self._sa(y, 0, line[:width-1], attr)

    # ── SCENARIO 1: Single bordered reading column ────────────────────────────
    # Triggered when NEITHER nav nor rpanel is visible.
    # Draws a rounded box around the entire text area with symmetric padding.

    def _draw_text_scenario1(self, left: int, width: int, top: int, height: int):
        """1-page: bordered reading box, symmetric padding, full content area."""
        if width < 8: return

        zm     = ZOOM_LEVELS[self.cfg.get("zoom", ZOOM_DEFAULT)]
        # Inner margins: horizontal zoom padding + 2 for box border
        pad_x  = zm + 2
        pad_y  = 1
        box_x  = left + zm
        box_w  = max(8, width - zm * 2)
        box_h  = height

        # Draw rounded border
        ac = self._cp("accent", bold=True)
        # Top border with chapter title
        ch_title, _ = self.chapters[self.ch_idx]
        title_str = f" {ch_title} "
        border_top = BOX["tl"] + BOX["h"] * (box_w - 2) + BOX["tr"]
        # Embed title in top border
        if len(title_str) < box_w - 4:
            mid = (box_w - len(title_str)) // 2
            border_top = (BOX["tl"] + BOX["h"] * (mid - 1) +
                          title_str +
                          BOX["h"] * (box_w - 2 - mid - len(title_str) + 1) +
                          BOX["tr"])
        self._sa(top,          box_x, border_top[:box_w],  ac)
        self._sa(top + box_h - 1, box_x,
                 BOX["bl"] + BOX["h"] * (box_w - 2) + BOX["br"], ac)
        for r in range(1, box_h - 1):
            self._sa(top + r, box_x,           BOX["v"], ac)
            self._sa(top + r, box_x + box_w - 1, BOX["v"], ac)

        # Inner text area
        inner_x = box_x + pad_x
        inner_w = max(4, box_w - pad_x * 2)
        inner_h = box_h - 2   # minus top and bottom border rows
        inner_top = top + pad_y + 1  # +1 for top border row

        lines = self._get_lines(inner_w)
        vis   = inner_h - pad_y  # leave bottom padding
        maxs  = max(0, len(lines) - vis)
        self.view_scroll = min(self.view_scroll, maxs)

        hl = -1
        if self._highlight and self.player.state in ("playing", "paused") and self.player.est_char_pos > 0:
            hl = self._char_to_line(self.player.est_char_pos, lines)
            if self._autoscroll:
                mg = 3
                if hl < self.view_scroll + mg:
                    self.view_scroll = max(0, hl - mg)
                elif hl >= self.view_scroll + vis - mg:
                    self.view_scroll = min(maxs, hl - vis + mg + 1)

        for row in range(vis):
            lidx = self.view_scroll + row
            y    = inner_top + row
            if y >= top + box_h - 1: break
            if lidx >= len(lines): break
            line = lines[lidx]
            if not line: continue
            is_head = (lidx == 0 or (lidx > 0 and not lines[lidx-1].strip()))
            if lidx == hl:
                self._sa(y, inner_x, line.ljust(inner_w)[:inner_w], self._cp("hl", bold=True))
            elif is_head:
                self._sa(y, inner_x, line[:inner_w], self._cp("accent", bold=True))
            else:
                self._sa(y, inner_x, line[:inner_w], self._cp("normal"))

        # Scroll % in bottom-right of box
        if len(lines) > vis:
            pct = int(self.view_scroll / max(1, maxs) * 100)
            self._sa(top + box_h - 1, box_x + box_w - 6,
                     f"{pct:3d}%", self._cp("dim"))

    # ── SCENARIO 2: Two snake-flow columns ────────────────────────────────────
    # Triggered when exactly ONE panel is visible.
    # Text fills left column then continues in right column (snake-flow).

    def _draw_text_scenario2(self, left: int, width: int, top: int, height: int, W: int):
        """2-page snake layout: text flows left col → right col."""
        if width < 16: return

        zm     = ZOOM_LEVELS[self.cfg.get("zoom", ZOOM_DEFAULT)]
        # Divider in the centre of the text area
        col_w  = (width - 1) // 2   # each column width (1 col for divider)
        col_pad = max(0, zm)
        inner_w = max(4, col_w - col_pad * 2 - 1)

        col1_x = left + col_pad
        col2_x = left + col_w + 1 + col_pad
        div_x  = left + col_w

        lines = self._get_lines(inner_w)
        vis_rows = height - 1   # one row for chapter title at top

        # Draw divider
        ch_title, _ = self.chapters[self.ch_idx]
        self._sa(top, left, f" ── {ch_title} "[:width-1], self._cp("accent", bold=True))
        for r in range(vis_rows):
            self._sa(top + 1 + r, div_x, BOX["v"], self._cp("accent"))

        # Column headers
        self._sa(top, col1_x, "① ", self._cp("dim"))
        self._sa(top, col2_x, "② ", self._cp("dim"))

        total_vis  = vis_rows * 2   # total visible lines across both columns
        maxs       = max(0, len(lines) - total_vis)
        self._snake_scroll = min(self._snake_scroll, maxs)

        hl = -1
        if self._highlight and self.player.state in ("playing", "paused") and self.player.est_char_pos > 0:
            hl = self._char_to_line(self.player.est_char_pos, lines)
            # Autoscroll: keep hl visible across the combined 2-column view
            if self._autoscroll:
                mg = 3
                # Absolute position of hl in snake flow
                if hl < self._snake_scroll + mg:
                    self._snake_scroll = max(0, hl - mg)
                elif hl >= self._snake_scroll + total_vis - mg:
                    self._snake_scroll = min(maxs, hl - total_vis + mg + 1)

        self._render_snake_columns(
            lines, top + 1, vis_rows,
            [(col1_x, inner_w), (col2_x, inner_w)],
            self._snake_scroll, hl
        )

        # Scroll %
        if len(lines) > total_vis:
            pct = int(self._snake_scroll / max(1, maxs) * 100)
            self._sa(top, left + width - 6, f"{pct:3d}%", self._cp("dim"))

    # ── SCENARIO 3: Three snake-flow columns ──────────────────────────────────
    # Triggered when BOTH nav and rpanel are visible.
    # Text flows col1 → col2 → col3 (snake-flow across 3 columns).

    def _draw_text_scenario3(self, left: int, width: int, top: int, height: int, W: int):
        """3-page snake layout: text flows across 3 equal columns."""
        if width < 24: return

        zm     = ZOOM_LEVELS[self.cfg.get("zoom", ZOOM_DEFAULT)]
        # 3 columns with 2 dividers
        # Width budget: width - 2 dividers
        col_w   = (width - 2) // 3
        col_pad = max(0, zm // 2)   # gentler zoom for 3-col — less aggressive padding
        inner_w = max(4, col_w - col_pad * 2 - 1)

        col1_x  = left + col_pad
        div1_x  = left + col_w
        col2_x  = div1_x + 1 + col_pad
        div2_x  = left + col_w * 2 + 1
        col3_x  = div2_x + 1 + col_pad

        lines = self._get_lines(inner_w)
        vis_rows = height - 1

        ch_title, _ = self.chapters[self.ch_idx]
        self._sa(top, left, f" ── {ch_title} "[:width-1], self._cp("accent", bold=True))

        # Dividers
        for r in range(vis_rows):
            self._sa(top + 1 + r, div1_x, BOX["v"], self._cp("accent"))
            self._sa(top + 1 + r, div2_x, BOX["v"], self._cp("accent"))

        # Column markers
        self._sa(top, col1_x, "① ", self._cp("dim"))
        self._sa(top, col2_x, "② ", self._cp("dim"))
        self._sa(top, col3_x, "③ ", self._cp("dim"))

        total_vis  = vis_rows * 3
        maxs       = max(0, len(lines) - total_vis)
        self._snake_scroll = min(self._snake_scroll, maxs)

        hl = -1
        if self._highlight and self.player.state in ("playing", "paused") and self.player.est_char_pos > 0:
            hl = self._char_to_line(self.player.est_char_pos, lines)
            if self._autoscroll:
                mg = 3
                if hl < self._snake_scroll + mg:
                    self._snake_scroll = max(0, hl - mg)
                elif hl >= self._snake_scroll + total_vis - mg:
                    self._snake_scroll = min(maxs, hl - total_vis + mg + 1)

        self._render_snake_columns(
            lines, top + 1, vis_rows,
            [(col1_x, inner_w), (col2_x, inner_w), (col3_x, inner_w)],
            self._snake_scroll, hl
        )

        if len(lines) > total_vis:
            pct = int(self._snake_scroll / max(1, maxs) * 100)
            self._sa(top, left + width - 6, f"{pct:3d}%", self._cp("dim"))

    # ── Snake column renderer (shared by S2 and S3) ───────────────────────────

    def _render_snake_columns(self, lines: list, top: int, vis_rows: int,
                              columns: list, scroll: int, hl: int):
        """
        Render lines into N columns sequentially (snake-flow).
        columns: list of (x_pos, inner_width) tuples.
        scroll:  starting line index in `lines`.
        hl:      highlighted line index (absolute in `lines`), or -1.
        """
        for col_idx, (cx, cw) in enumerate(columns):
            for row in range(vis_rows):
                lidx = scroll + col_idx * vis_rows + row
                y    = top + row
                if lidx >= len(lines):
                    break
                line = lines[lidx]
                if not line:
                    continue
                is_head = (lidx == 0 or (lidx > 0 and not lines[lidx - 1].strip()))
                if lidx == hl:
                    self._sa(y, cx, line.ljust(cw)[:cw], self._cp("hl", bold=True))
                elif is_head:
                    self._sa(y, cx, line[:cw], self._cp("accent", bold=True))
                else:
                    self._sa(y, cx, line[:cw], self._cp("normal"))

    # ── Right panel: voices (engine + voice sub-sections) ────────────────────

    def _draw_rp_voices(self, left, width, top, height):
        focused = self.focus == "rpanel"

        # ── Header ───────────────────────────────────────────────────────────
        label = " ENGINES & VOICES" + (" ←" if focused else "")
        self._sa(top,   left, label,            self._cp("rpanel", bold=True))
        self._sa(top,   left+width-3, "Esc",   self._cp("dim"))
        self._sa(top+1, left, BOX["h"]*width,   self._cp("accent"))

        row = 2
        # ── Engine section ────────────────────────────────────────────────────
        self._sa(top+row, left, " ENGINE", self._cp("accent", bold=True))
        row += 1
        active_engine = self.cfg.get("engine", "edge-tts")

        for ei, eng_id in enumerate(ENGINE_ORDER):
            if top+row >= top+height-2: break
            meta    = ENGINE_META[eng_id]
            avail   = self._mgr.is_available(eng_id)
            is_sel  = (eng_id == active_engine)
            is_cur  = (ei == self._engine_sel)
            tick    = "✓" if is_sel else " "
            badge   = "○" if meta["type"] == "online" else "◉"
            label   = meta["label"]
            line    = f" {tick}{badge} {label:<12}"

            if not avail:
                attr = self._cp("dim")
                line = f"  {badge} {label:<12} ✗"
            elif is_cur and focused and self._rp_section == "engine":
                attr = self._cp("sel", bold=True)
            elif is_sel:
                attr = self._cp("ok", bold=True)
            elif meta["type"] == "online":
                attr = self._cp("online")
            else:
                attr = self._cp("offline")

            self._sa(top+row, left, line[:width], attr)
            row += 1

        # divider between engine and voice sections
        if top+row < top+height-2:
            self._sa(top+row, left, BOX["h"]*width, self._cp("dim"))
            row += 1

        # ── Voice section ─────────────────────────────────────────────────────
        if top+row < top+height-2:
            eng_for_voices = ENGINE_ORDER[self._engine_sel] if self._engine_sel < len(ENGINE_ORDER) else active_engine
            voices = get_voices_for_engine(eng_for_voices)

            self._sa(top+row, left,
                     f" VOICES ({eng_for_voices})"[:width],
                     self._cp("accent", bold=True))
            row += 1

            visible_voices = height - row - 2
            cur_vidx       = self.cfg.get("voice_index", 0)

            # Scroll rp_scroll in voice sub-section
            if self._rp_section == "voice":
                if self._voice_sel < self.rp_scroll:
                    self.rp_scroll = self._voice_sel
                elif self._voice_sel >= self.rp_scroll + visible_voices:
                    self.rp_scroll = self._voice_sel - visible_voices + 1
            else:
                self.rp_scroll = 0

            for vr in range(visible_voices):
                vidx = self.rp_scroll + vr
                y    = top + row + vr
                if y >= top+height-1 or vidx >= len(voices): break
                vname, vcountry, vgender, _ = voices[vidx]
                sym  = "♀" if vgender == "F" else "♂" if vgender == "M" else " "
                tick = "✓" if (vidx == cur_vidx and eng_for_voices == active_engine) else " "
                line = f" {tick}{sym} {vname:<14} {vcountry}"

                if vidx == self._voice_sel and focused and self._rp_section == "voice":
                    self._sa(y, left, line.ljust(width)[:width], self._cp("sel", bold=True))
                elif vidx == cur_vidx and eng_for_voices == active_engine:
                    self._sa(y, left, line[:width], self._cp("ok", bold=True))
                else:
                    self._sa(y, left, line[:width], self._cp("normal"))

        # Hint line
        hint = " ↑↓:nav  Tab:section  Enter:pick"
        self._sa(top+height-1, left, hint[:width], self._cp("dim"))

    # ── Right panel: bookmarks ────────────────────────────────────────────────

    def _draw_rp_bookmarks(self, left, width, top, height):
        focused = self.focus == "rpanel"
        hattr   = self._cp("accent", bold=True)
        bms     = self.cfg["bookmarks"].get(self._bm_key, [])
        label = f" BOOKMARKS ({len(bms)})"
        if focused:
            label += " ←"
        self._sa(top,   left, label,         hattr)
        self._sa(top,   left+width-3, "Esc", self._cp("dim"))
        self._sa(top+1, left, BOX["h"]*width, self._cp("accent"))

        if not bms:
            self._sa(top+2, left+1, "No bookmarks yet.", self._cp("dim"))
            self._sa(top+3, left+1, "Press b to add.",   self._cp("dim"))
        else:
            visible = height - 4
            if self.rp_sel < self.rp_scroll:
                self.rp_scroll = self.rp_sel
            elif self.rp_sel >= self.rp_scroll + visible:
                self.rp_scroll = self.rp_sel - visible + 1

            for row in range(visible):
                idx = self.rp_scroll + row
                y   = top + 2 + row
                if idx >= len(bms): break
                bm     = bms[idx]
                chi    = bm["chapter"]
                ctitle = self.chapters[chi][0] if chi < len(self.chapters) else "?"
                num    = f"ch{chi+1:>3}"
                disp   = ctitle[:width-8]
                line   = f" {num} {disp}"
                if idx == self.rp_sel and focused:
                    self._sa(y, left, line.ljust(width)[:width], self._cp("sel", bold=True))
                else:
                    self._sa(y, left, line[:width], self._cp("bm"))

        hint = " Enter=go  d=del  b=add"
        self._sa(top+height-1, left, hint[:width], self._cp("dim"))

    # ── Footer ────────────────────────────────────────────────────────────────

    def _draw_footer(self, W, H, fh):
        y = H - fh
        self._sa(y, 0, BOX["h"]*(W-1), self._cp("accent"))

        if self.player.state == "error" and self.player.error_msg:
            self._sa(y+1, 2, f"✗ {self.player.error_msg}"[:W//2],
                     self._cp("err", bold=True))
        elif self.player.state in ("loading", "generating"):
            total  = max(1, self.player.total_chunks)
            done   = self.player.chunk_idx
            bw     = min(28, W//3)
            filled = int(done/total*bw)
            bar    = "█"*filled + "░"*(bw-filled)
            lbl    = self.player.anim_frame if self.player.state == "generating" else "◌"
            self._sa(y+1, 2, f"{lbl} [{bar}] {done}/{total}",
                     self._cp("dim", bold=True))
        else:
            total_ch = len(self.chapters)
            bw     = min(28, W//3)
            filled = int(self.ch_idx/max(1,total_ch-1)*bw)
            bar    = "█"*filled + "░"*(bw-filled)
            self._sa(y+1, 2, f"Book [{bar}] {self.ch_idx+1}/{total_ch}",
                     self._cp("playing"))

        KEYS = [
            ("Spc", "play/pause"),  ("Enter", "play ch"),
            ("n/p",  "next/prev"),  ("\\",     "nav"),
            ("v",    "voices"),     ("B",      "bookmarks"),
            ("b",    "+bookmark"),  ("s",      "speed"),
            ("a",    "align"),      ("t",      "theme"),
            ("z",    "autoscroll"), ("h",      "highlight"),
            ("g",    "goto"),       ("C+scroll","zoom"),
        ]
        col_w   = 18
        cols    = 2
        rows_k  = (len(KEYS) + cols - 1) // cols
        box_w   = col_w * cols + 3
        box_h   = rows_k + 2

        available_rows = fh - 1
        if box_h > available_rows:
            box_h  = available_rows
            rows_k = box_h - 2

        bx = max(W//3, (W - box_w) // 2)
        by = y + 1

        if bx + box_w >= W - 14:
            bx = max(2, W - box_w - 15)

        ac = self._cp("accent")
        di = self._cp("dim")

        self._sa(by,          bx, BOX["tl"] + BOX["h"]*(box_w-2) + BOX["tr"], ac)
        self._sa(by+box_h-1,  bx, BOX["bl"] + BOX["h"]*(box_w-2) + BOX["br"], ac)
        for r in range(1, box_h-1):
            self._sa(by+r, bx,           BOX["v"], ac)
            self._sa(by+r, bx+box_w-1,   BOX["v"], ac)

        for r in range(rows_k):
            for c in range(cols):
                idx = r + c * rows_k
                if idx >= len(KEYS): continue
                k, action = KEYS[idx]
                cx = bx + 1 + c * (col_w + 1)
                ry = by + 1 + r
                if ry >= by + box_h - 1: continue
                key_lbl = f"[{k}]"
                self._sa(ry, cx,                key_lbl,               ac | curses.A_BOLD)
                self._sa(ry, cx+len(key_lbl)+1, action[:col_w-len(key_lbl)-2], di)

        btn_w  = 11
        rx     = W - btn_w - 2
        if rx > bx + box_w + 1:
            dbg_attr = self._cp("playing", bold=True) if self._debug else self._cp("accent", bold=True)
            dbg_lbl  = "? debug" if not self._debug else "? DEBUG"
            self._sa(by,          rx, BOX["tl"] + BOX["h"]*(btn_w-2) + BOX["tr"], self._cp("accent"))
            self._sa(by+1,        rx, BOX["v"], self._cp("accent"))
            self._sa(by+1,        rx+1, f" {dbg_lbl} ".center(btn_w-2)[:btn_w-2], dbg_attr)
            self._sa(by+1,        rx+btn_w-1, BOX["v"], self._cp("accent"))
            self._sa(by+2,        rx, BOX["ml"] + BOX["h"]*(btn_w-2) + BOX["mr"], self._cp("accent"))
            self._sa(by+3,        rx, BOX["v"], self._cp("accent"))
            self._sa(by+3,        rx+1, " q quit ".center(btn_w-2)[:btn_w-2], self._cp("err", bold=True))
            self._sa(by+3,        rx+btn_w-1, BOX["v"], self._cp("accent"))
            self._sa(by+4,        rx, BOX["bl"] + BOX["h"]*(btn_w-2) + BOX["br"], self._cp("accent"))

    # ── Floating prev/next ────────────────────────────────────────────────────

    def _draw_float_btns(self, W, H, fh):
        y = H - fh - 2
        if y < 4: return
        prev = " ◀ prev "
        nxt  = " next ▶ "
        x    = W - len(prev) - len(nxt) - 3
        if x < 2: return
        pa = self._cp("btn", bold=True) if self.ch_idx > 0                    else self._cp("dim")
        na = self._cp("btn", bold=True) if self.ch_idx < len(self.chapters)-1 else self._cp("dim")
        self._sa(y, x,               prev, pa)
        self._sa(y, x+len(prev)+1,   nxt,  na)

    # ── Debug overlay ─────────────────────────────────────────────────────────

    def _draw_debug(self, H, W):
        p      = self.player
        bms    = self.cfg["bookmarks"].get(self._bm_key, [])
        H2, W2 = self.scr.getmaxyx()
        dh     = 17
        dy     = max(3, H2 - dh - 4)
        dw     = min(80, W2 - 4)
        dx     = max(0, (W2 - dw) // 2)

        self._sa(dy,      dx, BOX["tl"]+BOX["h"]*(dw-2)+BOX["tr"], self._cp("playing", bold=True))
        self._sa(dy+dh-1, dx, BOX["bl"]+BOX["h"]*(dw-2)+BOX["br"], self._cp("playing", bold=True))
        for r in range(1, dh-1):
            self._sa(dy+r, dx,      BOX["v"], self._cp("playing", bold=True))
            self._sa(dy+r, dx+dw-1, BOX["v"], self._cp("playing", bold=True))
        self._sa(dy, dx+2, " DEBUG  (? to close) ", self._cp("playing", bold=True))

        eng_avail = {k: v for k, v in self._mgr.availability.items()}
        engine = self.cfg.get('engine', 'edge-tts')
        voice  = current_voice_id(self.cfg)
        speed  = SPEEDS[self.cfg['speed_index']]
        pl_status = "  ".join(
            f"ch{self.ch_idx+1+i}:{self._preload.status(self.ch_idx+i, engine, voice, speed)}"
            for i in range(1, PRELOAD_AHEAD+1)
            if self.ch_idx+i < len(self.chapters)
        )
        rows = [
            f"  File       : {self.filename}",
            f"  Chapter    : {self.ch_idx+1}/{len(self.chapters)}  scroll={self.view_scroll}  snake={self._snake_scroll}",
            f"  Scenario   : {self._reading_scenario()}  nav={self.nav_visible}  rpanel={self.rpanel}",
            f"  Player     : state={p.state}  chunk={p.chunk_idx}/{p.total_chunks}",
            f"  Playback   : est_pos={p.est_char_pos}  ch_len={p.chapter_len}  cps={p._chars_per_sec:.1f}",
            f"  Paused     : total_paused={p._paused_total:.1f}s",
            f"  Engine     : {engine}  voice={voice}",
            f"  Availability: {eng_avail}",
            f"  Preload    : {pl_status or 'n/a'}",
            f"  Config     : speed={speed}×  "
                           f"align={self.cfg.get('align')}  "
                           f"zoom={self.cfg.get('zoom')}  "
                           f"theme={self.cfg.get('theme')}",
            f"  Nav        : nav_vis={self.nav_visible}  rpanel={self.rpanel}  focus={self.focus}",
            f"  RP section : {self._rp_section}  engine_sel={self._engine_sel}  voice_sel={self._voice_sel}",
            f"  Autoscroll : {self._autoscroll}  Highlight: {self._highlight}",
            f"  Bookmarks  : {len(bms)} for this file",
            f"  Last key   : {self._last_key}",
            f"  Log        : {LOG_PATH}",
        ]
        for i, row in enumerate(rows[:dh-2]):
            self._sa(dy+1+i, dx+1, row[:dw-2], self._cp("normal"))

    # ── Mouse ─────────────────────────────────────────────────────────────────

    def _handle_mouse(self):
        try:
            _, mx, my, _, bstate = curses.getmouse()
        except curses.error:
            return
        H, W = self.scr.getmaxyx()

        ctrl_scroll_up   = (bstate & curses.BUTTON4_PRESSED) and (bstate & curses.BUTTON_CTRL)
        ctrl_scroll_down = (bstate & curses.BUTTON5_PRESSED) and (bstate & curses.BUTTON_CTRL)
        plain_up         = (bstate & curses.BUTTON4_PRESSED) and not (bstate & curses.BUTTON_CTRL)
        plain_down       = (bstate & curses.BUTTON5_PRESSED) and not (bstate & curses.BUTTON_CTRL)

        self._last_mouse = (f"x={mx} y={my} "
                            f"cu={bool(ctrl_scroll_up)} cd={bool(ctrl_scroll_down)} "
                            f"pu={bool(plain_up)} pd={bool(plain_down)}")

        if ctrl_scroll_up:
            zi = self.cfg.get("zoom", ZOOM_DEFAULT)
            if zi > 0:
                self.cfg["zoom"] = zi - 1
                save_config(self.cfg)
                self._invalidate()
            _kitty_font(_KITTY_INC)
            return
        if ctrl_scroll_down:
            zi = self.cfg.get("zoom", ZOOM_DEFAULT)
            if zi < len(ZOOM_LEVELS) - 1:
                self.cfg["zoom"] = zi + 1
                save_config(self.cfg)
                self._invalidate()
            _kitty_font(_KITTY_DEC)
            return

        nav_w, tl, tw, rpl, rpw, hh, fh, ch = self._layout(H, W)
        scenario = self._reading_scenario()

        if plain_up or plain_down:
            delta = -3 if plain_up else 3
            if self.rpanel and mx >= rpl:
                bms   = self.cfg["bookmarks"].get(self._bm_key, [])
                limit = max(0, (len(get_voices_for_engine(self.cfg.get("engine","edge-tts")))
                                if self.rpanel=="voices" else len(bms)) - 1)
                self.rp_sel = max(0, min(limit, self.rp_sel + delta))
            elif self.nav_visible and mx < nav_w:
                self.ch_idx = max(0, min(len(self.chapters)-1, self.ch_idx + delta))
                self._invalidate()
            else:
                # Text area scroll — scenario-aware
                if scenario == 1:
                    lines = self._tlines if self._tlines else []
                    maxs  = max(0, len(lines)-5)
                    self.view_scroll = max(0, min(maxs, self.view_scroll + delta))
                else:
                    lines = self._tlines if self._tlines else []
                    n_cols = 2 if scenario == 2 else 3
                    H2, W2 = self.scr.getmaxyx()
                    vis_rows = ch - 1
                    total_vis = vis_rows * n_cols
                    maxs = max(0, len(lines) - total_vis)
                    self._snake_scroll = max(0, min(maxs, self._snake_scroll + delta))

    # ── Keyboard ──────────────────────────────────────────────────────────────

    def _handle_key(self, key):
        if key == -1: return
        try:
            self._last_key = f"{key} ({chr(key) if 32<=key<127 else '?'})"
        except Exception:
            self._last_key = str(key)

        if self.focus == "rpanel" and self.rpanel:
            if self._handle_rp_key(key):
                return

        if key in (ord('q'), ord('Q')):
            self._preload.invalidate()
            self.player.stop(); self.running = False; return

        if key == ord('?'):
            self._debug = not self._debug; return

        if key in (ord('h'), ord('H')):
            self._highlight = not self._highlight; return

        if key == ord(' '):
            if self.player.state in ("playing","paused"):
                self.player.toggle_pause()
            else:
                self._play()
            return

        if key in (curses.KEY_ENTER, 10, 13):
            self.player.stop(); self._play(); return

        if key == ord('\t'):
            order = ["chapters","text"]
            if self.rpanel: order.append("rpanel")
            cur = self.focus if self.focus in order else "chapters"
            self.focus = order[(order.index(cur)+1) % len(order)]
            return

        if key == ord('\\'):
            self.nav_visible = not self.nav_visible
            self._invalidate(); return

        if key in (ord('v'), ord('V')):
            if self.rpanel == "voices":
                self.rpanel = None
                self.focus  = "text"
            else:
                self.rpanel = "voices"
                self._rp_section = "engine"
                self._engine_sel = ENGINE_ORDER.index(self.cfg.get("engine","edge-tts"))
                self._voice_sel  = self.cfg.get("voice_index", 0)
                self.rp_scroll   = 0
                self.focus       = "rpanel"
            self._invalidate()
            return

        if key in (ord('B'),):
            if self.rpanel == "bookmarks":
                self.rpanel = None
                self.focus  = "text"
            else:
                self.rpanel = "bookmarks"
                self.rp_sel = 0
                self.rp_scroll = 0
                self.focus  = "rpanel"
            self._invalidate()
            return

        if key == 27:
            if self.rpanel:
                self.rpanel = None
                self.focus  = "text"
                self._invalidate()
            return

        if key in (ord('n'), ord('N')):
            if self.ch_idx < len(self.chapters)-1:
                self.player.stop()
                self.ch_idx += 1
                self._invalidate(); self._play()
            return

        if key in (ord('p'), ord('P')):
            if self.ch_idx > 0:
                self.player.stop()
                self.ch_idx -= 1
                self._invalidate(); self._play()
            return

        if key in (ord('s'), ord('S')):
            was = self.player.state in ("playing","paused","loading","generating")
            self.cfg["speed_index"] = (self.cfg["speed_index"]+1) % len(SPEEDS)
            save_config(self.cfg)
            self._preload.invalidate()
            if was: self.player.stop(); self._play()
            return

        if key == ord('b'):
            bms = self.cfg["bookmarks"].setdefault(self._bm_key, [])
            if not any(bm["chapter"] == self.ch_idx for bm in bms):
                bms.append({"chapter": self.ch_idx, "note": ""})
                save_config(self.cfg)
                log.info("bookmark added ch=%d", self.ch_idx)
            return

        if key in (ord('a'), ord('A')):
            cur = self.cfg.get("align","left")
            self.cfg["align"] = ALIGN_ORDER[(ALIGN_ORDER.index(cur)+1) % len(ALIGN_ORDER)]
            save_config(self.cfg)
            self._invalidate(); return

        if key in (ord('t'), ord('T')):
            cur = self.cfg.get("theme","terminal")
            idx = THEME_ORDER.index(cur) if cur in THEME_ORDER else 0
            self.cfg["theme"] = THEME_ORDER[(idx+1) % len(THEME_ORDER)]
            save_config(self.cfg)
            self._setup_colors(); return

        if key in (ord('z'), ord('Z')):
            self._autoscroll = not self._autoscroll; return

        if key in (ord('g'), ord('G')):
            self._goto_chapter(); return

        # Arrow keys — scenario-aware scrolling
        scenario = self._reading_scenario()

        if self.focus == "chapters":
            if key == curses.KEY_UP and self.ch_idx > 0:
                self.ch_idx -= 1; self._invalidate()
            elif key == curses.KEY_DOWN and self.ch_idx < len(self.chapters)-1:
                self.ch_idx += 1; self._invalidate()
        elif self.focus == "rpanel" and self.rpanel:
            self._handle_rp_key(key)
        else:
            if scenario == 1:
                # Single-column scroll
                lines = self._tlines if self._tlines else []
                maxs  = max(0, len(lines)-5)
                if key == curses.KEY_UP:
                    self.view_scroll = max(0, self.view_scroll-1)
                elif key == curses.KEY_DOWN:
                    self.view_scroll = min(maxs, self.view_scroll+1)
                elif key == curses.KEY_PPAGE:
                    self.view_scroll = max(0, self.view_scroll-20)
                elif key == curses.KEY_NPAGE:
                    self.view_scroll = min(maxs, self.view_scroll+20)
            else:
                # Multi-column snake scroll
                H, W = self.scr.getmaxyx()
                _, _, _, _, _, hh, fh, ch_h = self._layout(H, W)
                vis_rows  = ch_h - 1
                n_cols    = 2 if scenario == 2 else 3
                lines     = self._tlines if self._tlines else []
                total_vis = vis_rows * n_cols
                maxs      = max(0, len(lines) - total_vis)
                step      = vis_rows  # one full column page
                if key == curses.KEY_UP:
                    self._snake_scroll = max(0, self._snake_scroll - 1)
                elif key == curses.KEY_DOWN:
                    self._snake_scroll = min(maxs, self._snake_scroll + 1)
                elif key == curses.KEY_PPAGE:
                    self._snake_scroll = max(0, self._snake_scroll - step)
                elif key == curses.KEY_NPAGE:
                    self._snake_scroll = min(maxs, self._snake_scroll + step)

    def _handle_rp_key(self, key) -> bool:
        """Handle key when right panel is focused. Returns True if consumed."""
        if key == 27:
            self.rpanel = None; self.focus = "text"
            self._invalidate()
            return True

        if self.rpanel == "voices":
            num_engines = len(ENGINE_ORDER)

            if self._rp_section == "engine":
                if key == curses.KEY_UP:
                    self._engine_sel = max(0, self._engine_sel - 1); return True
                if key == curses.KEY_DOWN:
                    self._engine_sel = min(num_engines-1, self._engine_sel + 1); return True
                if key == ord('\t') or key == curses.KEY_RIGHT:
                    # Switch to voice sub-section
                    self._rp_section = "voice"
                    self._voice_sel  = self.cfg.get("voice_index", 0)
                    self.rp_scroll   = 0
                    return True
                if key in (curses.KEY_ENTER, 10, 13):
                    # Select engine
                    eng = ENGINE_ORDER[self._engine_sel]
                    if self._mgr.is_available(eng):
                        was = self.player.state in ("playing","paused","loading","generating")
                        self.cfg["engine"]      = eng
                        self.cfg["voice_index"] = 0
                        self._voice_sel         = 0
                        self.rp_scroll          = 0
                        save_config(self.cfg)
                        self._preload.invalidate()
                        log.info("engine changed to %s", eng)
                        if was: self.player.stop(); self._play()
                    return True

            elif self._rp_section == "voice":
                eng_for_voices = ENGINE_ORDER[self._engine_sel]
                voices         = get_voices_for_engine(eng_for_voices)
                if key == curses.KEY_UP:
                    self._voice_sel = max(0, self._voice_sel - 1); return True
                if key == curses.KEY_DOWN:
                    self._voice_sel = min(max(0,len(voices)-1), self._voice_sel + 1); return True
                if key == ord('\t') or key == curses.KEY_LEFT:
                    # Switch back to engine section
                    self._rp_section = "engine"; return True
                if key in (curses.KEY_ENTER, 10, 13):
                    if voices and self._voice_sel < len(voices) and voices[0][3] != "":
                        was = self.player.state in ("playing","paused","loading","generating")
                        self.cfg["engine"]      = eng_for_voices
                        self.cfg["voice_index"] = self._voice_sel
                        save_config(self.cfg)
                        self._preload.invalidate()
                        log.info("voice changed: engine=%s voice=%s",
                                 eng_for_voices, voices[self._voice_sel][0])
                        if was: self.player.stop(); self._play()
                    return True

        elif self.rpanel == "bookmarks":
            bms = self.cfg["bookmarks"].get(self._bm_key, [])
            if key == curses.KEY_UP:
                self.rp_sel = max(0, self.rp_sel-1); return True
            if key == curses.KEY_DOWN:
                self.rp_sel = min(max(0,len(bms)-1), self.rp_sel+1); return True
            if key in (curses.KEY_ENTER, 10, 13):
                if bms and self.rp_sel < len(bms):
                    target = bms[self.rp_sel]["chapter"]
                    self.player.stop()
                    self.ch_idx = target
                    self._invalidate()
                    self.rpanel = None; self.focus = "text"
                    self._play()
                return True
            if key in (ord('d'), ord('D')):
                if bms and self.rp_sel < len(bms):
                    bms.pop(self.rp_sel)
                    self.rp_sel = max(0, self.rp_sel-1)
                    self.cfg["bookmarks"][self._bm_key] = bms
                    save_config(self.cfg)
                    log.info("bookmark deleted idx=%d", self.rp_sel)
                return True

        return False

    # ── Goto chapter inline prompt ────────────────────────────────────────────

    def _goto_chapter(self):
        H, W = self.scr.getmaxyx()
        curses.curs_set(1)
        prompt = f" Go to chapter (1–{len(self.chapters)}): "
        self._sa(H-2, 0, " "*(W-1), self._cp("ftr"))
        self._sa(H-2, 0, prompt, self._cp("ftr", bold=True))
        self.scr.refresh()
        buf = ""
        while True:
            k = self.scr.getch()
            if k in (curses.KEY_ENTER, 10, 13): break
            elif k == 27: buf = ""; break
            elif k in (curses.KEY_BACKSPACE, 127): buf = buf[:-1]
            elif ord('0') <= k <= ord('9'): buf += chr(k)
            self._sa(H-2, len(prompt), buf.ljust(6), self._cp("ftr", bold=True))
            self.scr.refresh()
        curses.curs_set(0)
        if buf:
            try:
                n = int(buf)-1
                if 0 <= n < len(self.chapters):
                    self.player.stop(); self.ch_idx = n
                    self._invalidate(); self._play()
            except ValueError:
                pass


# ─── File picker ─────────────────────────────────────────────────────────────

def pick_file(stdscr):
    curses.curs_set(0)
    curses.start_color(); curses.use_default_colors()
    curses.init_pair(1, curses.COLOR_WHITE, -1)
    curses.init_pair(2, curses.COLOR_CYAN,  -1)
    curses.init_pair(3, curses.COLOR_BLACK,  curses.COLOR_CYAN)
    curses.init_pair(8, curses.COLOR_WHITE,  curses.COLOR_BLUE)
    cwd, sel, scroll = Path.home(), 0, 0

    def entries(d):
        out = []
        try:
            if d != d.parent: out.append(("..", True, d.parent))
            for p in sorted(d.iterdir(), key=lambda p:(not p.is_dir(), p.name.lower())):
                if not p.name.startswith(".") and (p.is_dir() or p.suffix.lower() in (".epub",".txt")):
                    out.append((p.name, p.is_dir(), p))
        except PermissionError: pass
        return out

    elist = entries(cwd)
    while True:
        stdscr.erase()
        H, W = stdscr.getmaxyx()
        try:
            stdscr.addstr(0, 0, " "*(W-1), curses.color_pair(8)|curses.A_BOLD)
            stdscr.addstr(0, 0, f"  📖 readaloud — {cwd}"[:W-1], curses.color_pair(8)|curses.A_BOLD)
            stdscr.addstr(1, 0, "─"*(W-1), curses.color_pair(2))
        except curses.error: pass
        vis = H-5
        if sel < scroll: scroll = sel
        elif sel >= scroll+vis: scroll = sel-vis+1
        for row in range(vis):
            idx = scroll+row
            if idx >= len(elist): break
            name, is_dir, _ = elist[idx]
            pre  = "📁 " if is_dir else "📄 "
            line = f"  {pre}{name}"
            attr = (curses.color_pair(3)|curses.A_BOLD if idx==sel
                    else curses.color_pair(2) if is_dir
                    else curses.color_pair(1))
            try:
                stdscr.addstr(2+row, 0,
                    line[:W-1].ljust(W-1) if idx==sel else line[:W-1], attr)
            except curses.error: pass
        try:
            stdscr.addstr(H-2, 0, "  ↑↓:nav  Enter:open  q:quit"[:W-1], curses.color_pair(2))
        except curses.error: pass
        stdscr.refresh()
        k = stdscr.getch()
        if k == ord('q'): return None
        elif k == curses.KEY_UP:   sel = max(0, sel-1)
        elif k == curses.KEY_DOWN: sel = min(len(elist)-1, sel+1)
        elif k in (curses.KEY_ENTER, 10, 13) and elist:
            name, is_dir, path = elist[sel]
            if is_dir: cwd=path; elist=entries(cwd); sel=scroll=0
            else: return str(path)


# ─── Main ─────────────────────────────────────────────────────────────────────

def main():
    check_deps()
    cfg = load_config()

    # Initialise the backend manager early so scan happens before curses starts
    mgr = get_manager()
    log.info("Available engines: %s", mgr.available_engines())

    # Ensure configured engine is actually available; fall back to edge-tts
    if not mgr.is_available(cfg.get("engine", "edge-tts")):
        available = mgr.available_engines()
        cfg["engine"] = available[0] if available else "edge-tts"
        log.warning("Configured engine unavailable, using: %s", cfg["engine"])

    start_debug = "--debug" in sys.argv
    args = [a for a in sys.argv[1:] if not a.startswith("--")]

    if args:
        filepath = args[0]
    else:
        filepath = curses.wrapper(pick_file)
        if not filepath:
            print("No file selected."); sys.exit(0)

    if not os.path.exists(filepath):
        print(f"File not found: {filepath}"); sys.exit(1)

    ext = Path(filepath).suffix.lower()
    print(f"Parsing {Path(filepath).name}…", end="", flush=True)
    try:
        if ext == ".epub":   chapters = parse_epub(filepath)
        elif ext == ".txt":  chapters = parse_txt(filepath)
        else: print(f"\nUnsupported: {ext}"); sys.exit(1)
    except Exception as e:
        print(f"\nParse error: {e}")
        log.exception("parse error")
        sys.exit(1)

    if not chapters:
        print("\nNo readable chapters found."); sys.exit(1)
    print(f" {len(chapters)} chapters.")
    cfg["last_file"] = filepath
    save_config(cfg)
    log.info("starting TUI: %s  %d chapters  debug=%s", filepath, len(chapters), start_debug)

    def _run(s):
        TUI(s, chapters, filepath, cfg, start_debug=start_debug).run()

    curses.wrapper(_run)

if __name__ == "__main__":
    main()
